...
exports.paginate = (totalItems, itemsPerPage, currentPage, url, param_name) => {
     ...
     */
    let neighbours = 2; // items shown arround current page
    const html = [];
    html.push('<ul class="pagination">');
    /**
     * Modify neighbours to avoid having few buttons:
     *      - If there is no space for neighbours on the left, I'll show more on the right.
     *      - If there is no space for neighbours on the right, I'll show more on the left.
     */
    ...
    // Previous pages: between the first and the middle pages
    if (currentPage - neighbours > 2) {
    ...
    if (currentPage + neighbours < total - 1) {
        const n = Math.trunc((1 + currentPage + neighbours + total) / 2);
        url = addPagenoToUrl(url, n, param_name);
        html.push(`<li><a href="${url}">${n}</a></li>`);
    }
    // Last page
